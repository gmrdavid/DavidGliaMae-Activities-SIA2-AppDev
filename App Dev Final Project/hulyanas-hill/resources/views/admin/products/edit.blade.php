...
<div class="min-h-screen bg-gray-50 font-sans text-gray-900">
    ...
    <main class="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div class="mb-10">
            ...
            <h1 class="text-4xl font-bold text-gray-900 tracking-tight">Edit Product</h1>
            <p class="text-lg text-gray-500 mt-2">Update the details for <span class="font-bold text-gray-900">{{ $product->name }}</span>.</p>
        </div>

        <!-- FORM CARD -->
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm">
            <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data" class="p-8 sm:p-10 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" 
                              class="block w-full px-4 py-3.5 border border-gray-300 rounded-lg text-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-black focus:border-black @error('description') border-red-500 ring-1 ring-red-500 @enderror">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-base text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="category" id="category" 
                            ...
                            <option value="bfast" {{ $product->category == 'bfast' ? 'selected' : '' }}>All Day Breakfast</option>
                ...
                <div>
                    ...
                    @if($product->image)
                        ...
                        <div class="mb-4 p-4 bg-gray-100 rounded-lg border border-gray-200 flex items-center gap-4">
                            <img src="{{ asset('storage/' . $product->image) }}" alt="Current Image" class="w-20 h-20 object-cover rounded-lg border border-gray-300">
                            <div class="flex-1">
                                <p class="text-base font-semibold text-gray-900">Current Image</p>
                                <p class="text-sm text-gray-500">Leave empty to keep this image</p>
                            </div>
                        </div>
                    @endif
                ...
                <div class="flex items-center pt-2">
                    <button type="button" 
                            ...
                            class="relative inline-flex h-7 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 {{ old('is_active', $product->is_active) ? 'bg-black' : 'bg-gray-200' }}"
                            role="switch" aria-checked="{{ old('is_active', $product->is_active) ? 'true' : 'false' }}">
                        <span class="sr-only">Enable product</span>
                        <span aria-hidden="true" class="pointer-events-none inline-block h-6 w-6 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {{ old('is_active', $product->is_active) ? 'translate-x-5' : 'translate-x-0' }}"></span>
                    </button>
                    <input type="checkbox" name="is_active" id="is_active" value="1" class="hidden" {{ old('is_active', $product->is_active) ? 'checked' : '' }}>
